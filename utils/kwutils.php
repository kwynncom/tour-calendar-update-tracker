<?php // Kwynn Buess 2020/02/28 8:05pm - EST, America/New_York
...
class kwmoncli extends MongoDB\Client {
    public function __construct($dbpathin = '') {
	if ($dbpathin) $dbpath = $dbpathin;
	else           $dbpath = 'mongodb://127.0.0.1/';
	
	parent::__construct($dbpath, [], ['typeMap' => ['array' => 'array','document' => 'array', 'root' => 'array']]);
    }
...
class dao_generic { // dao = data access object, and a nod to east Asian Daoism.
    ...
    public function __construct($dbname, $dbpath = false) {
	$this->dbname = $dbname;
	$this->client = new kwmoncli($dbpath);
    }
...
/* HISTORY
 * 02/28 8:05pm - MongoDB - added option for another IP address / port
 * 
 * 2020/02/17 7:41pm EST, America/New_York - updated user agent in kwua()
 * 
